...
class MB_ConvTasNet(nn.Module):
    def __init__(self,
        ...
        self.w1 = nn.Parameter(th.randn(1,requires_grad=True))
        self.w2 = nn.Parameter(th.randn(1,requires_grad=True))
        self.w3 = nn.Parameter(th.randn(1,requires_grad=True))
        self.w4 = nn.Parameter(th.randn(1,requires_grad=True))
        # output 1x1 conv
        # n x B x T => n x N x T
        # NOTE: using ModuleList not python list
        # self.conv1x1_2 = th.nn.ModuleList(
    ...
    def forward(self, x):
        ...
        y3 = self.repeats3(y)
        y4 = self.repeats4(y)
        y = y1*self.w1 + y2*self.w2 + \
             y3*self.w3 + y4*self.w4
        # n x 2N x T
        e = th.chunk(self.mask(y), self.num_spks, 1)
        # n x N x T
        if self.non_linear_type == "softmax":
