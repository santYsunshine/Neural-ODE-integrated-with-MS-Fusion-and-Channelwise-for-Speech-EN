...
class Trainer(object):
    def __init__(self,
        ...
        if resume:
            ...
            '''
            # load nnet
            th.distributed.init_process_group(backend='nccl', init_method='tcp://localhost:23456', rank=0, world_size=1)
            nnet = nnet.to(self.device)
            self.nnet = th.nn.parallel.DistributedDataParallel(nnet,device_ids=self.gpuid)
            
            nnet.load_state_dict(cpt["model_state_dict"], False)
