...
class MS_SL2_split_model(nn.Module):
    def __init__(self,
        ...
        self.X = X
        self.slice = Slice
        self.ode_block = ODEBlock(ODEFunc(Sc, H))
    ...
    def forward(self, x):
        ...
        s = [x.permute(0, 2, 1).contiguous().view(-1, x.size(1)) for x in s]
        print("Shape of s after permute and view:", [x.shape for x in s])
        s = [self.ode_block(x) for x in s]
        s = [x.view(-1, w.size(2), x.size(1)).permute(0, 2, 1) for x in s]
