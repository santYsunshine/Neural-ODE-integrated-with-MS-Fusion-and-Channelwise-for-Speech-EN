...
def build_audio(is_clean, params, index, audio_samples_length=-1):
    ...
    if is_clean:
        ...
        #     source_files = np.array(noisefilesSelected['filepath'].tolist())
    ...
    while remaining_length > 0 and tries_left > 0:
        ...
        print('source_files[idx]: ', source_files[idx])
    ...
    if tries_left == 0 and not is_clean in params.keys():
        print("There are not enough non-clipped files in the "  + \
...
def main_gen(params):
    ...
    while file_num <= params['fileindex_end']:
        ...
        if noise_index//len(params['noisefilenames']) > 0 and noise_index%len(params['noisefilenames']) == 0:
        ...
        cleanfilename = clean_files_joined + '_' + noise_files_joined + '_snr' + \
                        str(snr) + '_tl' + str(target_level) + '_fileid_' + str(file_num) + '.wav'
        noisefilename = clean_files_joined + '_' + noise_files_joined + '_snr' + \
                        str(snr) + '_tl' + str(target_level) + '_fileid_' + str(file_num) + '.wav'
...
def balance_noise(noisefilenames_df, balance_type='over_sampling'):
    noisetypes = list(set(noisefilenames_df['type'].tolist()))
    print('noisetypes: ', noisetypes)
    noisefilenames_dfgroup = noisefilenames_df.groupby('type')
    max_num = noisefilenames_dfgroup.size().max()
    min_num = noisefilenames_dfgroup.size().min()
    print('max: ', max_num)
    print('min: ', min_num)
    df_out = pd.DataFrame(columns=noisefilenames_df.columns)
    if balance_type == 'over_sampling':
        for noisetype in noisetypes:
            noisetype = str(noisetype)
            type_df = noisefilenames_df.query('type==@noisetype')
            if len(type_df['type']) == max_num:
                df_out = pd.concat([df_out, type_df], ignore_index=True)
            else:
                type_df = type_df.sample(max_num, replace=True)
                df_out = pd.concat([df_out, type_df], ignore_index=True)
    elif balance_type == 'under_sampling':
        for noisetype in noisetypes:
            noisetype = str(noisetype)
            type_df = noisefilenames_df.query('type==@noisetype')
            if len(type_df['type']) == min_num:
                df_out = pd.concat([df_out, type_df], ignore_index=True)
            else:
                type_df = type_df.sample(min_num, replace=True)
                df_out = pd.concat([df_out, type_df], ignore_index=True)
    return df_out
...
def main_body():
    ...
    if 'noise_csv' in cfg.keys() and cfg['noise_csv'] != 'None':
        print('read noise csv')
        noisefilenames_df = pd.read_csv(cfg['noise_csv'], header=None, names=['type', 'filepath'])
        noisefilenames_df = balance_noise(noisefilenames_df)
        noisefilenames = noisefilenames_df['filepath'].values
    else:
        noisefilenames = glob.glob(os.path.join(noise_dir, params['audioformat']))

    print('len noisefilenames: ', len(noisefilenames))
    # if 'noise_csv' not in cfg.keys():
    shuffle(noisefilenames)
    params['noisefilenames'] = noisefilenames
    # else:
    #     # noisedirs = glob.glob(os.path.join(noise_dir, '*'))
    #     print('read noise types')
    #     noisedirs = list(set(noisefilenames['type'].tolist()))
    ...
    #     print('noisedirs: ', noisedirs)
    #     shuffle(noisedirs)

    #     params['noisedirs'] = noisedirs
    #     params['noisefiles'] = noisefilenames
